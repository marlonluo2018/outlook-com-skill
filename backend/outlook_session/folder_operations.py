...
class FolderOperations:
    ...
    def get_folder_emails(self, folder_name: str = "Inbox", max_emails: int = 100, fast_mode: bool = True, days_filter: int = None) -> Tuple[List[Dict[str, Any]], str]:
        ...
        try:
            ...
            # Email list is ready to return (no cache needed)
            logger.info(f"Email extraction completed, returning {len(email_list)} emails")
...
def list_folders(hide_system_folders=False):
    """Get list of all folder names with hierarchical structure.
    
    Args:
        hide_system_folders: If True, hide system/hidden folders
    
    Returns:
        List of folder names with indentation showing hierarchy
    """
    from ..outlook_session.session_manager import OutlookSessionManager
    
    # System folders to hide when hide_system_folders=True
    SYSTEM_FOLDERS = {
        'PersonMetadata', 'Quick Step Settings', 'RSS Feeds',
        'Social Activity Notifications', 'Sync Issues', 'Yammer Root',
        'Conversation History', 'Files', 'ExternalContacts',
        'EventCheckPoints', 'Conversation Action Settings',
        'PeopleCentricConversation Buddies', 'Recipient Cache',
        'GAL Contacts', 'Organizational Contacts'
    }
    
    def _is_system_folder(folder_name):
        """Check if folder is a system folder."""
        return folder_name in SYSTEM_FOLDERS or folder_name.startswith('{')
    
    def _format_folder_tree(folder, indent=0, hide_system=False):
        """Recursively format folder tree with indentation."""
        result = []
        
        # Skip system folders if requested
        if hide_system and _is_system_folder(folder.Name):
            return result
        ...
        try:
            ...
            for subfolder in folder.Folders:
                result.extend(_format_folder_tree(subfolder, indent + 1, hide_system))
        except Exception as e:
            logger.warning(f"Error accessing subfolders of {folder.Name}: {str(e)}")
    ...
    try:
        with OutlookSessionManager() as session_manager:
            ...
            for folder in session_manager.outlook_namespace.Folders:
                all_folders.extend(_format_folder_tree(folder, 0, hide_system_folders))
            return all_folders
    except Exception as e:
         logger.error(f"Error getting folder list: {str(e)}")
         return []
